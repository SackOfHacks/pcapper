...
def _make_payloads(total: int, unique: int, size: int) -> list[bytes]:
    random.seed(1337)
    base = [os.urandom(size) for _ in range(max(1, unique))]
...
def main() -> int:
    ...
    parser.add_argument(
        "--total", type=int, default=20000, help="Total payloads per round"
    )
    ...
    print(
        f"Payloads: total={args.total} unique={args.unique} size={args.size} bytes rounds={args.rounds}"
    )
