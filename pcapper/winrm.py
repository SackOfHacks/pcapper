...
class WinrmSummary:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "winrm_packets": self.winrm_packets,
            ...
            "https_sessions": self.https_sessions,
            ...
            "http_hosts": dict(self.http_hosts),
            "user_agents": dict(self.user_agents),
            "auth_schemes": dict(self.auth_schemes),
            "soap_actions": dict(self.soap_actions),
            ...
            "conversations": [
                {
                    ...
                    "http_detected": conv.http_detected,
                    "https_detected": conv.https_detected,
                    "hosts": list(conv.hosts),
                }
...
def _scan_plaintext(
    ...
    for item in _extract_ascii_strings(payload):
        ...
        ua_match = USER_AGENT_RE.search(item)
        if ua_match:
            user_agents[ua_match.group(1).strip()] += 1
        auth_match = AUTH_RE.search(item)
        if auth_match:
            auth_value = auth_match.group(1).strip()
            scheme = auth_value.split()[0] if auth_value else auth_value
            if scheme:
                auth_schemes[scheme] += 1
        if WSMAN_RE.search(item):
...
def analyze_winrm(
    ...
    auth_schemes: Counter[str] = Counter()
    soap_actions: Counter[str] = Counter()
    ...
    try:
        for pkt in reader:
            ...
            if payload:
                ...
                for host in _extract_ascii_strings(payload):
                    host_match = HOST_RE.search(host)
                    if host_match:
                        session.hosts[host_match.group(1).strip()] += 1
    ...
    for session in sessions.values():
        conversations.append(
            WinrmConversation(
                ...
                http_detected=session.http_detected,
                https_detected=session.https_detected,
                hosts=list(session.hosts.keys()),
            )
        )
        if session.http_detected:
            http_sessions += 1
        if session.https_detected:
            https_sessions += 1
...
def merge_winrm_summaries(
    summaries: list[WinrmSummary] | tuple[WinrmSummary, ...] | set[WinrmSummary],
) -> WinrmSummary:
    ...
    for summary in summary_list:
        ...
        winrm_packets += summary.winrm_packets
        ...
        http_sessions += summary.http_sessions
        ...
        http_hosts.update(summary.http_hosts)
        user_agents.update(summary.user_agents)
        auth_schemes.update(summary.auth_schemes)
        soap_actions.update(summary.soap_actions)
